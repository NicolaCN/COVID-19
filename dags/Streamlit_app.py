...
# Read the data
cases_deaths_df = pd.read_csv('./files_production/cases_deaths.csv')
...
# Read the data
vaccinations_df = pd.read_csv('./files_production/vaccinations.csv')

# Create a copy of the dataframe
df = vaccinations_df.copy()

if measure_vaccine == "cumulative_vaccine_doses_administered_per_hundred":
    # Interpolate the cumulative feature columns for each country
    for country in df['Country_code'].unique():
        country_mask = df['Country_code'] == country
        df.loc[country_mask, "cumulative_vaccine_doses_administered_per_hundred"] = df.loc[country_mask, "cumulative_vaccine_doses_administered_per_hundred"].interpolate()
...
st.write("""### How did the vaccination campaign affect the evolution of the pandemic in terms of cases and deths?""")
